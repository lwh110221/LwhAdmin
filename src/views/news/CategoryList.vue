<template>
  <div class="container mx-auto px-4 py-8">
    ...
    <div class="flex justify-between items-center mb-6">
      <h2 class="text-2xl font-bold">新闻分类管理</h2>
      <button
        ...
        <table class="min-w-full divide-y divide-gray-200">
          <thead class="bg-gray-50">
            <tr>
              ...
              <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">分类代码</th>
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            ...
            <tr v-for="category in categoryList" :key="category.id" class="hover:bg-gray-50">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ category.code }}</td>
...
const validateForm = () => {
  ...
  if (!form.value.name) {
    ...
    errors.value.name = '分类名称长度必须在2-20个字符之间'
  }
  
  if (!form.value.code) {
    errors.value.code = '请输入分类代码'
  } else if (!/^[a-z][a-z0-9_]{1,19}$/.test(form.value.code)) {
    errors.value.code = '分类代码必须以小写字母开头，只能包含小写字母、数字、下划线，长度2-20'
  }
...
const fetchCategoryList = async () => {
  ...
  try {
    ...
    if (response?.code === 200) {
      ...
      logger.info('获取新闻分类列表成功', { total: categoryList.value.length })
    } else {
      showMessage(response?.message || '获取分类列表失败', 'error')
...
const handleSubmit = async () => {
  ...
  try {
    logger.info(`开始${isEdit.value ? '更新' : '创建'}新闻分类`, form.value)
    const response = isEdit.value
      ? await newsService.updateCategory(form.value.id, form.value)
      : await newsService.createCategory(form.value)

    if (response?.code === 200) {
      showMessage(`${isEdit.value ? '更新' : '创建'}成功`, 'success')
      logger.info(`${isEdit.value ? '更新' : '创建'}新闻分类成功`, form.value)
      showDialog.value = false
      fetchCategoryList()
    } else {
      showMessage(response?.message || `${isEdit.value ? '更新' : '创建'}失败`, 'error')
      logger.warn(`${isEdit.value ? '更新' : '创建'}新闻分类失败`, response?.message)
    }
  } catch (error) {
    showMessage(`${isEdit.value ? '更新' : '创建'}失败`, 'error')
    logger.error(`${isEdit.value ? '更新' : '创建'}新闻分类失败`, error)
  } finally {
...
const handleDelete = async (category) => {
  ...
  try {
    logger.info('开始删除新闻分类', { categoryId: category.id })
    const response = await newsService.deleteCategory(category.id)

    if (response?.code === 200) {
      showMessage('删除成功', 'success')
      logger.info('删除新闻分类成功', { categoryId: category.id })
      fetchCategoryList()
    } else {
      showMessage(response?.message || '删除失败', 'error')
      logger.warn('删除新闻分类失败', response?.message)
    }
  } catch (error) {
